...
func (cs *categoryService) Create(ctx context.Context, dto categoryDto.CategoryDTO) (int, error) {
	...
	if err != nil {
		fmt.Println(err)
		return 0, fmt.Errorf("ошибка при создании сущности категории")
	}
	return id, nil
}
...
func (cs *categoryService) Update(ctx context.Context, dto categoryDto.CategoryDTO) error {
	...
	if err != nil {
		fmt.Println(err)
		return fmt.Errorf("ошибка при обновлении сущности категории с id: %d", *dto.Id.Value)
	}
	return nil
}
...
func (cs *categoryService) Delete(ctx context.Context, id int) error {
	...
	if err != nil {
		fmt.Println(err)
		return fmt.Errorf("ошибка при удалении сущности категории с id: %d", id)
	}
	return nil
}
...
func (cs *categoryService) FindAll(ctx context.Context) ([]categoryDto.CategoryDTO, error) {
	...
	if err != nil {
		fmt.Println(err)
		return nil, fmt.Errorf("ошибка при получении списка записей категорий")
	}
...
func (cs *categoryService) FindOne(ctx context.Context, id int) (categoryDto.CategoryDTO, error) {
	...
	if err != nil {
		fmt.Println(err)
		return categoryDto.CategoryDTO{}, fmt.Errorf("ошибка при получении записи категории с id: %d", id)
	}
